		...
		for (const name of Object.keys(cookie_paths)) {
			cookie_paths[name] = new Set(
				[...cookie_paths[name]].filter(
					(path) =>
						!path_matches(normalized_url, path) || name in initial_cookies
				)
			);
		}
				...
				if (DEV) {
					...
					for (const layout of layouts) {
						if (layout) {
							...
							validate_common_exports(layout.shared, layout.shared_id);
						}
					}
					if (page) {
						validate_page_server_exports(page.server, page.server_id);
						validate_common_exports(page.shared, page.shared_id);
					}
				}
